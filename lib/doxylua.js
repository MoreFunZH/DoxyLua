module.exports = function (file, showAST, showLocations) {
    ...
    events.on('Comment', function _dealComment(node) {
        if (lastComment == null) {
            ...
            if (_getNodeByLine(node.loc.start.line, metaNodes) == null) {    
    ...
    // Prepare the meta nodes for dealing with comments.
    _prepareMetaNodes(nodes, metaNodes);
...
function _isTableEnumNode(node) {
    return node.variables.length == 1 && node.init.length == 1 && 
...
function _translateTableEnum(node, comments) {
    ...
    if (node.comment == null) {
        node.comment = [ '/// @enum ' + varName.name ];
    }
    else {
        node.comment[0] = node.comment[0].replace(/^\/+/, '/// @enum ' + varName.name);
    }
...
function _prepareMetaNodes(nodes, metaNodes) {
    for (var i = 0; i < nodes.length; ++i) {
        var node = nodes[i];
